...
该文档用于记录团队参与SH-HK跨学科社交媒体谣言监测Task1评测项目的过程及结果：
...
因此，该评测将在消息级别标注的PHEME数据集上对消息的引爆点类别进行预测，以期辅助后续的谣言检测。

成果：

1. 在baseline模型的基础上，增加了LSTM模块并添加了dropout机制，超过了基线模型。

2. 取得了在trigger子任务上超过基线模型2.36%，在verify子任务上超过基线模型16.03%，排行榜第二的成绩。
...
我们的目标是：在Macro F1上取得比baseline模型更好的效果（已达成）
...
我们通过实验和分析，最终采用增加一层LSTM并在最终分类时加入dropout的方式，来构建我们改进之后的模型：

![our_model](/Img/our_model.bmp)
