...
tweet_tokenizer = TweetTokenizer()  # 将单词流转换成小的标记，token
...
class TriggerDataModule(pl.LightningDataModule):
    ...
    def _load_file(self, filename, test=False):
        ...
        if test:
            ...
            df = df[fields]
            df['trigger'] = [0]*len(df)
            df['verify'] = [0]*len(df)  # 若是测试集test1/2，置trigger（无实意）和verify（虚假）值为0
        else:
    ...
    def _clean_sentence(self, sentence):
        ...
        sentence = re.sub('[\.\+\-\?\'\\,"《》‘=+_!！?*()/$%&#:;^_`{|}~><“”]', '', sentence)  # remove special tokens
